...
import {BoltIcon,HomeIcon,LeagueIcon,LogoutIcon,RouteIcon,SourceIcon,UserIcon} from './ui-icons';
...
export function StudentHeader(){
 ...
 return <>
  ...
  <nav className="mobile-bottom-nav grid-cols-5 md:hidden">{links.map(({href,label,icon:Icon})=><Link key={href} href={href} className={`mobile-nav-item ${pathname===href?'mobile-nav-item-active':''}`}><Icon className="h-5 w-5"/><span>{label}</span></Link>)}</nav>
 </>
}
